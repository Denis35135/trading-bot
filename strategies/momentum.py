"""
Strategie Momentum pour The Bot
...
class MomentumStrategy(BaseStrategy):
    ...
    - Entre sur acceleration du momentum
    - Profits vises: 1-3%
    - Holding: 15min  plusieurs heures
    """
    
    def __init__(self, config: Dict = None):
        """
        Initialise la strategie Momentum
        
        Args:
            config: Configuration personnalisee
        """
        # Configuration par defaut specifique au momentum
        ...
        # "tat specifique au momentum
        self.breakout_levels = {}
        self.trend_strength = {}
        self.momentum_score = {}
        
        logger.info("Strategie Momentum initialisee")
    
    def analyze(self, data: Dict) -> Optional[Dict]:
        """
        Analyse les donnees pour detecter des opportunites momentum
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Calcule les indicateurs pour la strategie momentum
    ...
    def _check_entry_conditions(self, df: pd.DataFrame, current: pd.Series, 
        ...
        """
        Verifie les conditions d'entree momentum
        ...
        if prev['macd'] < prev['macd_signal'] and current['macd'] > current['macd_signal']:
            if current['macd_hist'] > prev['macd_hist'] * 1.5:  # Acceleration
        ...
        if self.config['rsi_range'][0] < current['rsi'] < self.config['rsi_range'][1]:
            if current['rsi'] > prev['rsi'] + 5:  # RSI en acceleration
        ...
        if len(signals['long']) >= min_confirmations and long_score >= self.config['min_confidence']:
            ...
            # Calculer les targets bases sur ATR
            atr = current['atr']
            take_profit = entry_price * (1 + self.config['profit_target'])
            stop_loss = entry_price - (1.5 * atr)  # Stop base sur volatilite
            
            # Ajuster si resistance proche
    ...
    def _check_exit_conditions(self, symbol: str, df: pd.DataFrame, current: pd.Series) -> Optional[Dict]:
        """
        Verifie les conditions de sortie pour une position momentum
        ...
        if side == 'BUY':
            # Long: sortir si momentum devient negatif
        ...
        # Decision de sortie
        if exit_reasons:
            # Prendre la raison avec le score le plus eleve
    ...
    def _calculate_support_resistance(self, df: pd.DataFrame, window: int = 20):
        """
        Calcule les niveaux de support et resistance
        
        Args:
            df: DataFrame avec OHLCV
            window: Fenetre pour detecter les pivots
        """
        high = df['high'].values
        low = df['low'].values
